...
This deterministic Blender 5.x extension combines the semantic
exporter and GPL bake worker in `miku_shader_converter-2.2.11.zip`.
...
keeps the 2.2.11 controls for normal convention, displacement policy, advanced
...
`bpy.app.version == (5, 2, 0)`. Any Blender 5.x is allowed; versions other
than the certified 5.2.0 emit an unvalidated-version warning but are not
