    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 GenshinFrag(Varyings input) : SV_Target
            {
                ...
                float3 diffuse = Genshin_BodyDiffuse(baseSample.rgb, lightMap, input.vertexColor, normalWS, lightDirWS, mainLightColor, mainLight.shadowAttenuation, _BodyShadowSmooth, _InNight, TEXTURE2D_ARGS(_ShadowRampMap, sampler_ShadowRampMap));
                #if defined(_AREA_SKIN)
                    diffuse = Genshin_ReferenceSkinTone(diffuse);
                #endif
                float3 indirect = Genshin_SampleSH_Indirect(normalWS) * _IndirectLightUsage * baseSample.rgb;
                float3 specular = Genshin_ComputeSpecular(baseSample.rgb, lightMap, metalMap, normalWS, viewDirWS, lightDirWS, _StrokeRange, _PatternRange, _MetalIntensity);
